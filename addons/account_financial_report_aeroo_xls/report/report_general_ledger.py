...
class Parser(report_sxw.rml_parse, common_report_header):
    ...
    def compute_currency(self, to_currency, from_currency, amt):
        ...
        if to_currency:
            curr_current = to_currency
        amount = currency_obj.compute(self.cr, self.uid, curr_current, from_currency, amt)
        return amount
    
    def get_symbol(self, currency_id, company_currency_id):
        cur_id = company_currency_id
        if currency_id:
            cur_id = currency_id
        currency_obj = self.pool.get('res.currency')
        return currency_obj.read(self.cr, self.uid, cur_id, ['symbol'])['symbol']
