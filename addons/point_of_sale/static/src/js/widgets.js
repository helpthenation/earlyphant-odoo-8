function openerp_pos_widgets(instance, module){ //module is instance.point_of_sale
    ...
    module.ProductCategoriesWidget = module.PosBaseWidget.extend({
        ...
        get_number_customer:function () {
            var order  = this.pos.get('selectedOrder');
            return order.number_customer;
        },
        //end adding
        ...
        render_category: function( category, with_image ){
            ...
            $('.set_table_number input',this.el).on('focusout', function (e){
                var order = self.pos.get('selectedOrder');
                if (this.value == ''){
                    $(".set_table_number input").css("box-shadow", "0px 0px 0px 0px rgb(181, 255, 153) inset");
                }else{
                    $(".set_table_number input").css("box-shadow", "0px 0px 0px 3px rgb(181, 255, 153) inset");
                }
                order.set_table_number(this.value);
                document.querySelector('.order-table-number').innerText = this.value;
            });

            $('.set_number_customer input',this.el).on('focusout', function (e){

                var order = self.pos.get('selectedOrder');
                if (this.value == ''){
                    $(".set_number_customer input").css("box-shadow", "0px 0px 0px 0px rgb(181, 255, 153) inset");
                }else{
                    $(".set_number_customer input").css("box-shadow", "0px 0px 0px 3px rgb(181, 255, 153) inset");
                }
                order.set_number_customer(this.value);
            });
            return cached; 
        },
