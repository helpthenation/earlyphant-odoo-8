		...
		for order in self.browse(cr, uid, ids, context=context):
			...
			def insert_data(data_type, values):
				...
				if data_type == 'product':
					...
					key = ('discount', values['partner_id'], values['account_id'], values['debit'] > 0)
				else:
					return
			...
			for line in order.lines:
				...
				if line.discount:
					dis += line.price_unit * line.qty * (line.discount/100)
					if line.product_id.categ_id.customer_discount_categ.id:
						discount_account = line.product_id.categ_id.customer_discount_categ.id
					elif line.product_id.customer_discount.id:
						discount_account = line.product_id.categ_id.customer_discount.id
					else:
						raise osv.except_osv(_('Error!'), _('Please define discount '\
				...
				while computed_taxes:
					...
					if tax_code_id:
						break
                #full_price
				full_price = line.price_unit * line.qty
				...
				insert_data('product', {
                    ...
                    'credit': ((full_price>0) and full_price) or 0.0,
                    'debit': ((full_price<0) and -full_price) or 0.0,
			...
			if dis != 0.0:
				insert_data('discount', {
                    'name': _("Customer Discount"), #order.name,
                    'account_id': discount_account,
                    'credit': ((dis < 0) and -dis) or 0.0,
                    'debit': ((dis > 0) and dis) or 0.0,
                    'partner_id': order.partner_id and self.pool.get("res.partner")._find_accounting_partner(order.partner_id).id or False
                })
            # counterpart
...
class ProductCategory(models.Model):
	...
	_columns = {
		'customer_discount_categ':fields.many2one('account.account', string='Customer Discount'),
	}

class ProductTemplate(models.Model):
	_inherit = "product.template"
	_columns = {
		'customer_discount':fields.many2one('account.account', string='Customer Discount')
	}
