...
class HotelReservation(models.Model):
    ...
    _defaults = {
        'checkin': _defaults_checkin_date,
        'checkout': _defaults_checkout_date
    }
    ...
    def _create_folio(self):
        ...
        for reservation in self:
            ...
            reservation.write({'state': 'done','check_write':True})
        return True
...
class RoomReservationSummary(models.Model):
    ...
    def generate_summary(self):
        ...
        if self.date_from and self.date_to:
            ...
            room_ids = []
            if self.room_type_summary.id == False:
                room_ids = room_obj.search([('categ_id','!=',10)])
            else:
                room_ids = room_obj.search([('categ_id', '=', self.room_type_summary.id)])
            #####
                   ...
                   not room.room_line_ids:
                    ...
                    for chk_date in date_range_list:
                        ...
                        if reservline_ids or folio_resrv_ids:
                            room_list_stats.append({'state': reservline_ids.reservation_id.partner_id.name or folio_resrv_ids.folio_id.partner_id.name,
                                                    'date': chk_date,
                                                    'room_id': room.id,
                                                    'is_draft': 'No',
                                                    'data_model': '',
