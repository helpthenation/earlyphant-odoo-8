...
class MartketListGeneralRequest(models.Model):
    _name = 'marketlist.general.request'
    _description = 'Market List Request'
    _inherit = ['mail.thread', 'ir.needaction_mixin']

    _track = {
        'state': {
            'market_list.vkg_request_to_approve':
                lambda self, cr, uid, obj,
                ctx=None: obj.state == 'to_approve',
            'market_list.vkg_request_approved':
                lambda self, cr, uid, obj,
                ctx=None: obj.state == 'approved',
            'market_list.vkg_request_rejected':
                lambda self, cr, uid, obj,
    ...
    def copy(self, default=None):
        ...
        return super(MartketListGeneralRequest, self).copy(default)
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].get('general.request.sequence')
        ...
        return super(MartketListGeneralRequest, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(MartketListGeneralRequest, self).write(vals)
    ...
    product_line_ids = fields.One2many('marketlist.request.general.line', 'request_id',
