...
class pos_detail(report_sxw.rml_parse):
    ...
    def _get_pos_reference(self, form):
        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids, context=self.localcontext):
            if pos.number_of_customer:
                if pos.number_of_customer.isdigit():
                    no_customer += int(pos.number_of_customer)
            i+=1
        data[0] = i
        data[1] = no_customer
        return data

    def _pos_order_line_1(self, form):
        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids, context=self.localcontext):
            result = {
                ...
                'pos_name': pos.name,
                'amount_tax': pos.amount_tax,
                'price_subtotal':(pos.amount_total-pos.amount_tax),
                'number_of_customer': pos.number_of_customer,
                'amount_total': pos.amount_total,
                'pos_reference': pos.pos_reference
                }
            data.append(result)
    ...
    def _pos_sales_details(self, form):
        ...
        data = []
        result = {}
        # location_id = form['locatoin_id'] or self._get_all_locations()
        # or self._get_all_users()
        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids, context=self.localcontext):
            for pol in pos.lines:
                ...
                data.append(result)
                #self.total_subtotal += result['price_subtotal']
                # self.total += result['price_subtotal']
    ...
    def _get_sum_invoice_2(self, form):
        ...
        company_id = user_obj.browse(self.cr, self.uid, self.uid).company_id.id
        date_start, date_end = self._get_utc_time_range(form)
        pos_ids = pos_obj.search(self.cr, self.uid, [('date_order', '>=', date_start), ('date_order', '<=', date_end),
                                                     ('location_id', 'in', [form['location_id'][0]]),
                                                     ('invoice_id', '<>', False)])
    ...
    def _get_tax_amount(self, form):
        ...
        for order in pos_order_obj.browse(self.cr, self.uid, pos_ids):
            for line in order.lines:
                line_taxes = account_tax_obj.compute_all(self.cr, self.uid, line.product_id.taxes_id,
    ...
    def get_location_id(self, form):

        location_obj = self.pool.get('stock.location')
        location_ids = location_obj.browse(self.cr, self.uid, [form['location_id'][0]])
        self.location = location_ids.name
        return self.location

    def __init__(self, cr, uid, name, context):
        super(pos_detail, self).__init__(cr, uid, name, context=context)
        self.total = 0.0
        self.qty = 0.0
        ...
        self.localcontext.update({
            ...
            'get_pos_reference': self._get_pos_reference,
            'getqtytotal2': self._get_qty_total_2,
            'getsalestotal2': self._get_sales_total_2,
            'pos_order_line_1':self._pos_order_line_1,
            ...
            'get_location_id': self.get_location_id

        })


class report_pos_details(osv.AbstractModel):
    _name = 'report.report_sale_location.report_detailsofsales_location'
    _inherit = 'report.abstract_report'
    _template = 'report_sale_location.report_detailsofsales_location'
    _wrapped_report_class = pos_detail
