...
class HotelRestaurantOrder(models.Model):
    ...
    def done_cancel(self):
        ...
        for line in self.folio_id.service_lines:
            if line.product_id.name in names:
                line.unlink()
    ...
    def unlink(self):
        for rec in self:
            raise ValidationError(_('You can not delete Table Order in %s\
                                state.') % (rec.state))
        return super(HotelRestaurantOrder,self).unlink()

    @api.multi
    def get_tax_default(self):
        return 10
...
class HotelRestaurantOrderList(models.Model):
    ...
    def _sub_total(self):
    	for line in self:
    		subtotal = line.item_rate * line.item_qty
    ...
    item_qty = fields.Float('Qty', size=64, required=True)
